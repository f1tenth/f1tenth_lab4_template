"""What the autograder runs on levine_blocked (the three timed laps):

    ros2 launch gap_follow levine_blocked_launch.py

Everything this map needs goes here: one node or several, Python or C++, and
the parameter values that suit this map (levine_obs_launch.py is the
obstacle course's). Start your own nodes only: the simulator is already running.
"""
